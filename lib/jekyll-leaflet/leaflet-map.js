...
// that is visible on the page
    ...
    function _addGeoJSONObjToMap(leafletItem, map, astyle){
    	 astyle = (typeof astyle !== 'undefined') ? astyle : {"color": "#0000ff","weight": 3,"opacity": 0.8};
         console.log("style is",leafletItem.style);
        ...
        for(var i=0; i<layers.length; i++){
            try{
                ...
                if(!('center' in tagInputArg)){
                    ...
                    console.log("track type is "+String(geom.type));
                    var pcoord;
                    if (geom.type.startsWith("Multi")){
                       pcoord=geom.coordinates[0][0];
                    } else {
                       pcoord=geom.coordinates[0];
                    };
                    console.log("panning to map coord " + pcoord);
                    map.panTo(new L.LatLng(pcoord[1],
                                           pcoord[0]));
                }
            } catch(e){}
        }
